...
from .model_util import TransformerBlock, LooseBlock, init_weights

class LooseModel(nn.Module):
    def __init__(self, config):
        ...
        self.dual_blocks = nn.ModuleList([
            LooseBlock(config) for _ in range(config.n_dual_blocks)
        ])
    ...
    def forward(self, x, targets=None, ignore_index=-1):
        ...
        secondary = self.secondary_embedding(x)
